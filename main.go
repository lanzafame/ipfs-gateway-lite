...
func createNode(ctx context.Context, repoPath string) (icore.CoreAPI, error) {
	...
	fmt.Println("ID", node.Identity)
	fmt.Println("Addrs", node.PeerHost.Addrs())
...
func connectToPeers(ctx context.Context, ipfs icore.CoreAPI, peers []string) error {
	...
	for _, peerInfo := range peerInfos {
		go func(peerInfo *peerstore.PeerInfo) {
			...
			if err != nil {
				log.Warningf("failed to connect to %s: %s", peerInfo.ID, err)
			}
		}(peerInfo)
	}
	wg.Wait()
...
func main() {
	...
	*/
	...
	err = Gateway(ipfs, mux, false, "/ipfs", "/ipns")
	if err != nil {
		panic("gateway constructor failed")
	}
